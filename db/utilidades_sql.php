...
function set_formulario_pagos ($_id_apartamento , $usuario , $concepto , $fecha_cobro , $total_pagar ) {

	global $MySqli;

	$sql = "INSERT INTO formulario_pagos (_id_apartamento ,  usuario , concepto , fecha_cobro , total_pagar  ) VALUES ('{$_id_apartamento}' , '{$usuario}' , '{$concepto}' , '{$fecha_cobro}' , '{$total_pagar}'  ) ";
	echo $sql;
	return $MySqli -> query($sql);
...
function set_formulario_vis ($_id_apartamento , $nombre , $apellido , $identificacion , $celular , $email, $fecha_ingreso, $foto ) {

	global $MySqli;

	$sql = "INSERT INTO formulario_vis (_id_apartamento , nombre ,apellido , identificacion , celular , email , fecha_ingreso, foto) VALUES ('{$_id_apartamento}' , '{$nombre}' ,'{$apellido}' ,'{$identificacion}' ,'{$celular}' ,'{$email}', '{$fecha_ingreso}', '{$foto}') ";
	echo $sql;	
	return $MySqli -> query($sql);
...
function set_empleados ($nombre , $apellido , $identificacion , $email , $funcion  ) {

	global $MySqli;

	$sql = "INSERT INTO empleados ( nombre , apellido, identificacion, email , funcion )
	VALUES ('{$nombre}' ,'{$apellido}','{$identificacion}','{$email}','{$funcion}') ";
	echo $sql;
	return $MySqli -> query($sql);
...
function get_visitantes(){

	global $MySqli;
	$sql = "SELECT apartamento.nombre AS nombreapartamento , bloque.nombre AS bloque , interior.nombre AS interior, formulario_vis.nombre,apellido,identificacion,celular,fecha_ingreso,foto FROM apartamento,bloque,interior,formulario_vis ORDER BY _id_formulario_vis DESC LIMIT 10 ";
	echo $sql;
	return $MySqli -> query($sql);
}


function set_reserva ($concepto  , $fecha_reserva   ) {

	global $MySqli;

	$sql = "INSERT INTO reserva ( concepto ,  fecha_reserva )
	VALUES ('{$concepto}' ,'{$fecha_reserva}' ";
	echo $sql;
	return $MySqli -> query($sql);
...
function set_contacto ($nombre , $email, $asunto, $mensaje   ) {
